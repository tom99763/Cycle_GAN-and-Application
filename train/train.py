...
def flow(x,y,l1,l2,Dx,Dy,Gxy,Gyx,Dx_opt,Dy_opt,Gxy_opt,Gyx_opt):
    ...
    Dx_grads = tape.gradient(Dx_loss*params.lambda_dis,Dx.trainable_variables)
    Dy_grads = tape.gradient(Dy_loss*params.lambda_dis, Dy.trainable_variables)
...
def main():
    ...
    for i in range(params.epochs):
        ...
        for x, y in loop:
            x, y = x[0][:, 0, ...], y[0][:, 0, ...]
            x, y = aug(x), aug(y)
            loss = flow(x, y, l1, l2, Dx, Dy, Gxy, Gyx, Dx_optim, Dy_optim, Gxy_optim, Gyx_optim)
